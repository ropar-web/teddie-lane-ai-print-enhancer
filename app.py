...
def main():
    ...
    with left:
        ...
        if total_pixels > AI_SAFE_OUTPUT_PIXELS:
            st.warning(
                "Large Gelato export: this can create the correct 300-DPI pixel size, "
                "but Real-ESRGAN AI is automatically disabled above 30 MP to stop Streamlit Cloud crashing. "
                "For true AI clarity at A1 300 DPI, use the local desktop version."
            )
    ...
    if target_w * target_h > MAX_OUTPUT_PIXELS:
        st.error("This output is larger than the Gelato cloud-safe limit. Use a smaller size or the local desktop version.")
        return
