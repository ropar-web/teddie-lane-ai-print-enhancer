...
AI_SAFE_OUTPUT_PIXELS = 30_000_000
...
def gelato_readiness(width_in: float, height_in: float, dpi: int) -> tuple[int, int, int, str]:
    px_w = round(width_in * dpi)
    px_h = round(height_in * dpi)
    total = px_w * px_h
    if dpi >= 300:
        status = "Gelato ideal target: 300 DPI"
    elif dpi >= 150:
        status = "Large-format minimum range: 150–299 DPI"
    else:
        status = "Below recommended print range"
    return px_w, px_h, total, status
...
def process_file(filename: str, data: bytes, spec: OutputSpec):
    ...
    report = {
        ...
        "DPI": spec.dpi,
        "AI": "Real-ESRGAN x2 Plus" if spec.use_ai else "Off or auto-disabled for large Gelato export",
...
def main():
    ...
    st.markdown(
        ...
        This update also includes <b>Gelato approval mode</b> for creating the real pixel dimensions Gelato checks.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left:
        ...
        target_w, target_h, total_pixels, gelato_status = gelato_readiness(width_in, height_in, int(dpi))
        c1, c2, c3 = st.columns(3)
        c1.metric("Pixels", f"{target_w:,} × {target_h:,}")
        c2.metric("DPI", dpi)
        c3.metric("Size", f"{total_pixels / 1_000_000:.1f} MP")
        st.caption(
            f"{gelato_status}. Use 300 DPI when Gelato requires it. "
            "A1 at 300 DPI is large, so the cloud app may need to turn AI off and use high-quality resize/export."
        )
        if total_pixels > AI_SAFE_OUTPUT_PIXELS:
            st.warning(
                "Large Gelato export: download is still allowed. Only the AI enhancement is automatically disabled above 30 megapixels to stop Streamlit Cloud crashing. "
                "You can still export up to 80 megapixels for Gelato approval. For true AI clarity at A1 300 DPI, use the local desktop version."
            )
    ...
    if target_w * target_h > MAX_OUTPUT_PIXELS:
        st.error("This output is above the 80-megapixel cloud export limit. Use a smaller size or the local desktop version.")
        return

    effective_use_ai = bool(use_ai)
    if target_w * target_h > AI_SAFE_OUTPUT_PIXELS:
        effective_use_ai = False
