...
def process_file(filename: str, data: bytes, spec: OutputSpec):
    ...
    for fmt in spec.output_formats:
        if fmt == "PDF":
            files.append(
                (
                    f"{stem}_AI_PRINT_READY_{spec.dpi}dpi.pdf",
                    image_to_pdf(output, spec.width_in, spec.height_in, spec.dpi),
                )
            )
        elif fmt == "PNG":
            files.append(
                (
                    f"{stem}_AI_PRINT_READY_{spec.dpi}dpi.png",
                    encode_image(output, "PNG", spec.dpi),
                )
            )
        elif fmt == "JPEG":
            files.append(
                (
                    f"{stem}_AI_PRINT_READY_{spec.dpi}dpi.jpg",
                    encode_image(output, "JPEG", spec.dpi),
                )
            )
...
def style():
    st.markdown(
        ...
        div[data-testid="stMetric"]{
          background:#FFF4E6;border:1px solid #E8D9BF;border-radius:14px;padding:9px
        }
        div.stButton>button,div.stDownloadButton>button{
          background:#7F946F;color:#fff;border:0;border-radius:12px;font-weight:700
        }
        .note{
          background:#FFF4E6;border-left:5px solid #CF6F4A;border-radius:12px;
...
def main():
    ...
    st.caption("Real-ESRGAN clarity enhancement with exact 300 DPI print export.")
    ...
    st.markdown(
        ...
        model directly through CPU PyTorch.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left:
        ...
        dpi = st.selectbox("Output DPI", [300, 350], index=0)
        fit_mode = st.radio(
            "Artwork fit",
